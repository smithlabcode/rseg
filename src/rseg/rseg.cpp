...

{
    try 
    {
        ...
        double posterior_cutoff = 0.95; // the posterior theshhold
                                       // above which a bin is
                                       // considerd belonging to a
                                       // state

        size_t undef_region_cutoff = 3000; // if an undefined region
        ...
        opt_parse.add_opt("smooth", '\0', "Indicate whether the rate curve is assumed smooth", false, smooth);
        ...
        opt_parse.add_opt("posterior", 'P', "Options for posterior decoding (default Viterbi)", false, use_posterior);
        ...
        opt_parse.add_opt("name", 'N', "Name of dataset (default: filename)", false, tmp_dataset_name);
        ...
        opt_parse.add_opt("read-counts-requested", 'C', "Write reads counts file in each bin",
                          false, Read_Counts_Requested);
//         opt_parse.add_opt("remove-jackpot", 'j', "Remove duplicate reads",
//                           false, Remove_Jackpot);
//         opt_parse.add_opt("both-domain-ends", '\0', "When boundaries must associated with domains",
//                           false, Both_Domain_Ends);
        ...
        LoadReadsByRegion(chroms_file.c_str(), reads_file.c_str(),
