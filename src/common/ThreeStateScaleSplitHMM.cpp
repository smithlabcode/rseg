...
double
ThreeStateScaleSplitHMM::ViterbiDecoding(
  const vector<double> &values, const std::vector<double> &scales,
  ...
  const SplitDistro &bg_distro, vector<size_t> &ml_classes) const {

  return ViterbiDecoding(values, scales, reset_points, start_trans[0],
                         start_trans[1], start_trans[2], trans[0][0],
                         trans[0][1], trans[0][2], trans[1][0], trans[1][1],
                         trans[1][2], trans[2][0], trans[2][1], trans[2][2],
                         end_trans[0], end_trans[1], end_trans[2], fg_distro,
                         mid_distro, bg_distro, ml_classes);
}

double
ThreeStateScaleSplitHMM::BaumWelchTraining(
  const vector<double> &values, const vector<double> &vals_a,
  const vector<double> &vals_b, const std::vector<double> &scales,
  const vector<size_t> &reset_points, vector<double> &start_trans,
  vector<vector<double>> &trans, vector<double> &end_trans,
  SplitDistro &fg_distro, SplitDistro &mid_distro,
  ...
  return BaumWelchTraining(values, vals_a, vals_b, scales, reset_points,
                           ...
                           trans[0][0], trans[0][1], trans[0][2], trans[1][0],
                           trans[1][1], trans[1][2], trans[2][0], trans[2][1],
                           trans[2][2], end_trans[0], end_trans[1],
                           end_trans[2], fg_distro, mid_distro, bg_distro);
}

double
ThreeStateScaleSplitHMM::PosteriorDecoding(
  const vector<double> &values, const std::vector<double> &scales,
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, vector<size_t> &classes,
  ...
  return PosteriorDecoding(values, scales, reset_points, start_trans[0],
                           ...
                           trans[1][2], trans[2][0], trans[2][1], trans[2][2],
                           end_trans[0], end_trans[1], end_trans[2], fg_distro,
                           mid_distro, bg_distro, classes, llr_scores);
}

void
ThreeStateScaleSplitHMM::PosteriorScores(
  const vector<double> &values, const std::vector<double> &scales,
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, const vector<size_t> &classes,
  vector<double> &llr_scores) const {
...
ThreeStateScaleSplitHMM::PosteriorScores(
  ...
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, const size_t class_id,
  vector<double> &llr_scores) const {
  ...
  return PosteriorScores(values, scales, reset_points, start_trans[0],
                         ...
                         trans[1][2], trans[2][0], trans[2][1], trans[2][2],
                         end_trans[0], end_trans[1], end_trans[2], fg_distro,
                         mid_distro, bg_distro, class_id, llr_scores);
}
...
ThreeStateScaleSplitHMM::PosteriorScores(
  ...
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, vector<double> &fg_scores,
  ...
  return PosteriorScores(values, scales, reset_points, start_trans[0],
                         ...
                         trans[1][2], trans[2][0], trans[2][1], trans[2][2],
                         end_trans[0], end_trans[1], end_trans[2], fg_distro,
                         mid_distro, bg_distro, fg_scores, bg_scores);
}

void
ThreeStateScaleSplitHMM::TransitionPosteriors(
  const vector<double> &values, const std::vector<double> &scales,
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, vector<vector<double>> &scores) const {

  return TransitionPosteriors(
    values, scales, reset_points, start_trans[0], start_trans[1],
    start_trans[2], trans[0][0], trans[0][1], trans[0][2], trans[1][0],
    trans[1][1], trans[1][2], trans[2][0], trans[2][1], trans[2][2],
    end_trans[0], end_trans[1], end_trans[2], fg_distro, mid_distro, bg_distro,
    scores);
}
...
double
ThreeStateScaleSplitHMM::forward_algorithm(
  const vector<double> &vals, const std::vector<double> &scales,
  const size_t start, const size_t end, const double lp_sf, const double lp_sm,
  const double lp_sb, const double lp_ff, const double lp_fm,
  const double lp_fb, const double lp_mf, const double lp_mm,
  const double lp_mb, const double lp_bf, const double lp_bm,
  const double lp_bb, const double lp_ft, const double lp_bt,
  const double lp_mt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
...
ThreeStateScaleSplitHMM::TransitionPosteriors(
  ...
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const SplitDistro &fg_distro, const SplitDistro &mid_distro,
  const SplitDistro &bg_distro, vector<vector<vector<double>>> &scores) const {
...
void
ThreeStateScaleSplitHMM::TransitionPosteriors(
  const vector<double> &values, const vector<double> &scales,
  const vector<size_t> &reset_points, const double p_sf, const double p_sm,
  const double p_sb, const double p_ff, const double p_fm, const double p_fb,
  const double p_mf, const double p_mm, const double p_mb, const double p_bf,
  const double p_bm, const double p_bb, const double p_ft, const double p_mt,
  const double p_bt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  ...
  assert(isfinite(lp_sf) && isfinite(lp_sm) && isfinite(lp_sb) &&
         isfinite(lp_ff) && isfinite(lp_fm) && isfinite(lp_fb) &&
         isfinite(lp_ft) && isfinite(lp_mf) && isfinite(lp_mm) &&
         isfinite(lp_mb) && isfinite(lp_mt) && isfinite(lp_bf) &&
         isfinite(lp_bm) && isfinite(lp_bb) && isfinite(lp_bt));
  ...
  for (size_t i = 0; i < reset_points.size() - 1; ++i) {
    ...
    const double score = forward_algorithm(
      values, scales, reset_points[i], reset_points[i + 1], lp_sf, lp_sm, lp_sb,
      lp_ff, lp_fm, lp_fb, lp_mf, lp_mm, lp_mb, lp_bf, lp_bm, lp_bb, lp_ft,
      lp_mt, lp_bt, fg_distro, mid_distro, bg_distro, forward);

    const double backward_score = backward_algorithm(
      values, scales, reset_points[i], reset_points[i + 1], lp_sf, lp_sm, lp_sb,
      lp_ff, lp_fm, lp_fb, lp_mf, lp_mm, lp_mb, lp_bf, lp_bm, lp_bb, lp_ft,
      lp_mt, lp_bt, fg_distro, mid_distro, bg_distro, backward);
  ...
  scores.resize(
    NUM_OF_STATES,
    vector<vector<double>>(NUM_OF_STATES, vector<double>(values.size(), 0)));
  size_t j = 0;
  for (size_t i = 0; i < values.size(); ++i) {
    if (i == reset_points[j]) {
      ...
      const double mid_to_fg_state =
        forward[i - 1][1] + lp_mf +
        fg_distro.log_likelihood(values[i], scales[i]) + backward[i][0];
      const double mid_to_mid_state =
        forward[i - 1][1] + lp_mm +
        mid_distro.log_likelihood(values[i], scales[i]) + backward[i][1];
      const double mid_to_bg_state =
        forward[i - 1][1] + lp_mb +
        bg_distro.log_likelihood(values[i], scales[i]) + backward[i][2];
      ...
      const double denom = log_sum_log(
        log_sum_log(fg_to_fg_state, fg_to_mid_state, fg_to_bg_state),
...
double
ThreeStateScaleSplitHMM::backward_algorithm(
  const vector<double> &vals, const std::vector<double> &scales,
  const size_t start, const size_t end, const double lp_sf, const double lp_sm,
  const double lp_sb, const double lp_ff, const double lp_fm,
  const double lp_fb, const double lp_mf, const double lp_mm,
  ...
  for (size_t k = end - 1; k > start; --k) {
    ...
    const double mid_a =
      mid_distro.log_likelihood(vals[k], scales[k]) + b[k][1];
    const double bg_a = bg_distro.log_likelihood(vals[k], scales[k]) + b[k][2];

    b[i][0] = log_sum_log(fg_a + lp_ff, mid_a + lp_fm, bg_a + lp_fb);

    b[i][1] = log_sum_log(fg_a + lp_mf, mid_a + lp_mm, bg_a + lp_mb);

    b[i][2] = log_sum_log(fg_a + lp_bf, mid_a + lp_bm, bg_a + lp_bb);
  }
  return log_sum_log(
    b[start][0] + fg_distro.log_likelihood(vals[start], scales[start]) + lp_sf,
...
void
ThreeStateScaleSplitHMM::estimate_transitions(
  const vector<double> &vals, const std::vector<double> &scales,
  ...
  vector<double> &ff_vals, vector<double> &fm_vals, vector<double> &fb_vals,

  vector<double> &mf_vals, vector<double> &mm_vals, vector<double> &mb_vals,

  vector<double> &bf_vals, vector<double> &bm_vals,
...
double
ThreeStateScaleSplitHMM::single_iteration(
  const vector<double> &values, const vector<double> &vals_a,
  const vector<double> &vals_b, const std::vector<double> &scales,
  const vector<size_t> &reset_points,

  vector<vector<double>> &forward, vector<vector<double>> &backward,

  double &p_sf, double &p_sm, double &p_sb, double &p_ff, double &p_fm,
  double &p_fb, double &p_mf, double &p_mm, double &p_mb, double &p_bf,
  double &p_bm, double &p_bb, double &p_ft, double &p_mt, double &p_bt,
  ...
  for (size_t i = 0; i < reset_points.size() - 1; ++i) {
    ...
    if (DEBUG &&
        fabs((score - backward_score) / max(score, backward_score)) > 1e-20) {
      cerr << "fabs(score - backward_score)/max(score, backward_score) > 1e-20"
           << endl;
      cerr << std::setprecision(12) << score << "\t" << std::setprecision(12)
           << backward_score << endl;
    }

    estimate_transitions(values, scales, reset_points[i], reset_points[i + 1],
                         forward, backward, score, lp_sf, lp_sm, lp_sb, lp_ff,
                         lp_fm, lp_fb, lp_mf, lp_mm, lp_mb, lp_bf, lp_bm, lp_bb,
                         lp_ft, lp_mt, lp_bt,

                         fg_distro, mid_distro, bg_distro,

                         ff_vals, fm_vals, fb_vals,

                         mf_vals, mm_vals, mb_vals,

                         bf_vals, bm_vals, bb_vals);
  ...
  //  SQ: ff_vals[reset_points[i] - 1] is always equal to 0, and should not be
  //  considered
  // i.e. the start of each region does not contribute to transition emission
  // estimate
  const double p_ff_new_estimate =
    exp(log_sum_log_vec(ff_vals, values.size())) - reset_points.size() + 1;
  const double p_fm_new_estimate =
    exp(log_sum_log_vec(fm_vals, values.size())) - reset_points.size() + 1;
  const double p_fb_new_estimate =
    exp(log_sum_log_vec(fb_vals, values.size())) - reset_points.size() + 1;

  const double p_mf_new_estimate =
    exp(log_sum_log_vec(mf_vals, values.size())) - reset_points.size() + 1;
  const double p_mm_new_estimate =
    exp(log_sum_log_vec(mm_vals, values.size())) - reset_points.size() + 1;
  const double p_mb_new_estimate =
    exp(log_sum_log_vec(mb_vals, values.size())) - reset_points.size() + 1;

  const double p_bf_new_estimate =
    exp(log_sum_log_vec(bf_vals, values.size())) - reset_points.size() + 1;
  const double p_bm_new_estimate =
    exp(log_sum_log_vec(bm_vals, values.size())) - reset_points.size() + 1;
  const double p_bb_new_estimate =
    exp(log_sum_log_vec(bb_vals, values.size())) - reset_points.size() + 1;
...
ThreeStateScaleSplitHMM::BaumWelchTraining(
  const vector<double> &values, const vector<double> &vals_a,
  const vector<double> &vals_b, const std::vector<double> &scales,
  const vector<size_t> &reset_points, double &p_sf, double &p_sm, double &p_sb,
  double &p_ff, double &p_fm, double &p_fb, double &p_mf, double &p_mm,
  double &p_mb, double &p_bf, double &p_bm, double &p_bb, double &p_ft,
  double &p_mt, double &p_bt, SplitDistro &fg_distro, SplitDistro &mid_distro,
  SplitDistro &bg_distro) const {
  ...
  if (VERBOSE)
    cout << setw(4) << "ITR" << setw(8) << "F BINS" << setw(8) << "M BINS"
         << setw(8) << "B BINS" << setw(32) << "F PARAMS" << setw(32)
         << "M PARAMS" << setw(32) << "B PARAMS" << setw(14) << "DELTA" << endl;
  ...
  for (size_t i = 0; i < max_iterations; ++i) {
    ...
    double total = single_iteration(
      values, vals_a, vals_b, scales, reset_points, forward, backward,

      p_sf, p_sm, p_sb, p_ff, p_fm, p_fb, p_mf, p_mm, p_mb, p_bf, p_bm, p_bb,
      p_ft, p_mt, p_bt,
    ...
    if (VERBOSE) {
      cout << setw(4) << i + 1 << setw(8) << 1.0 / (p_fb + p_fm) << setw(8)
           << 1.0 / (p_mf + p_mb) << setw(8) << 1.0 / (p_bf + p_bm) << setw(32)
           << fg_distro << setw(32) << mid_distro << setw(32) << bg_distro
           << setw(14) << (prev_total - total) / prev_total << "\t" << total
           << endl;
    }
...
ThreeStateScaleSplitHMM::PosteriorScores(
  ...
  const double p_bt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  const vector<size_t> &classes, vector<double> &llr_scores) const {
...
ThreeStateScaleSplitHMM::PosteriorScores(
  ...
  const double p_bt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  const size_t class_id, vector<double> &llr_scores) const {
...
ThreeStateScaleSplitHMM::PosteriorScores(
  ...
  const double p_bt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  vector<double> &fg_scores, vector<double> &bg_scores) const {
...
ThreeStateScaleSplitHMM::TransitionPosteriors(
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    if (i == reset_points[j]) {
      ...
      const double denom = log_sum_log(
        log_sum_log(log_sum_log(log_sum_log(fg_to_fg_state, fg_to_mid_state),
                                fg_to_bg_state),
...
ThreeStateScaleSplitHMM::PosteriorDecoding(
  ...
  const double p_bt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  vector<size_t> &classes, vector<double> &llr_scores) const {
  double total_score = 0;
...
ThreeStateScaleSplitHMM::ViterbiDecoding(
  ...
  const double p_sb, const double p_ff, const double p_fm, const double p_fb,
  const double p_mf, const double p_mm, const double p_mb, const double p_bf,
  const double p_bm, const double p_bb, const double p_ft, const double p_bt,
  const double p_mt, const SplitDistro &fg_distro,
  const SplitDistro &mid_distro, const SplitDistro &bg_distro,
  ...
  for (size_t i = 0; i < reset_points.size() - 1; ++i) {
    ...
    v.front()[0] =
      fg_distro.log_likelihood(values[start], scales[start]) + lp_sf;
    v.front()[1] =
      mid_distro.log_likelihood(values[start], scales[start]) + lp_sm;
    v.front()[2] =
      bg_distro.log_likelihood(values[start], scales[start]) + lp_sb;
