...
TwoStateScaleHMM::forward_algorithm(
  const vector<double> &vals, const vector<double> &scales, const size_t start,
  ...
  const double lp_bb, const double lp_bt, const Distro &fg_distro,
  const Distro &bg_distro, vector<pair<double, double>> &f) const {
...
TwoStateScaleHMM::backward_algorithm(
  ...
  const double lp_fb, const double lp_ft, const double lp_bf,
  const double lp_bb, const double lp_bt, const Distro &fg_distro,
  const Distro &bg_distro, vector<pair<double, double>> &b) const {
...
double
TwoStateScaleHMM::log_sum_log_vec(const vector<double> &vals,
                                  size_t limit) const {
...
TwoStateScaleHMM::single_iteration(
  ...
  //   const double p_ff_new_estimate = exp(log_sum_log_vec(ff_vals,
  //   values.size() - 1)); const double p_fb_new_estimate =
  //   exp(log_sum_log_vec(fb_vals, values.size() - 1)); const double
  //   p_bf_new_estimate = exp(log_sum_log_vec(bf_vals, values.size() - 1));
  //   const double p_bb_new_estimate = exp(log_sum_log_vec(bb_vals,
  //   values.size() - 1));
...
TwoStateScaleHMM::BaumWelchTraining(
  ...
  const std::vector<size_t> &reset_points, vector<double> &start_trans,
  vector<vector<double>> &trans, vector<double> &end_trans, Distro &fg_distro,
...
TwoStateScaleHMM::BaumWelchTraining(
  ...
  vector<pair<double, double>> forward(values.size(),
                                       pair<double, double>(0, 0));
  vector<pair<double, double>> backward(values.size(),
                                        pair<double, double>(0, 0));

  if (VERBOSE)
    cout << setw(5) << "ITR" << setw(10) << "F size" << setw(10) << "B size"
         << setw(18) << "F PARAMS" << setw(18) << "B PARAMS" << setw(14)
         << "DELTA" << endl;
  ...
  for (size_t i = 0; i < max_iterations; ++i) {
    ...
    if (VERBOSE) {
      cout << setw(5) << i + 1 << setw(10) << 1 / p_fb_est << setw(10)
           << 1 / p_bf_est << setw(18) << fg_distro.tostring() << setw(18)
           << bg_distro.tostring() << setw(14)
           << (prev_total - total) / prev_total << endl;
    }
...
TwoStateScaleHMM::PosteriorScores(
  ...
  const Distro &fg_distro, const Distro &bg_distro, const vector<bool> &classes,
...
TwoStateScaleHMM::PosteriorScores(
  ...
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const Distro &fg_distro, const Distro &bg_distro, const bool fg_class,
  vector<double> &llr_scores) const {
...
TwoStateScaleHMM::PosteriorScores(
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    const double bg_state = forward[i].second + backward[i].second;
    const double denom = log_sum_log(fg_state, bg_state);
    llr_scores[i] = fg_class ? exp(fg_state - denom) : exp(bg_state - denom);
  }
}
...
TwoStateScaleHMM::TransitionPosteriors(
  ...
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const Distro &fg_distro, const Distro &bg_distro, const size_t transition,
  vector<double> &llr_scores) const {
...
TwoStateScaleHMM::PosteriorDecoding(
  ...
  const vector<size_t> &reset_points, const vector<double> &start_trans,
  const vector<vector<double>> &trans, const vector<double> &end_trans,
  const Distro &fg_distro, const Distro &bg_distro, vector<bool> &classes,
  vector<double> &llr_scores) const {
