...
static void
partition_values(const vector<double> &values, vector<double> &fg_part, 
				 vector<double> &bg_part) {
		...
		for (; i < helper.size() && sum < val_per_part; ++i) {
				fg_part.push_back(helper[i]);
				sum += helper[i];
		}
		bg_part = vector<double>(helper.begin() + i, helper.end());
}
				 ...
				 vector<double> &fg_probs, vector<double> &bg_probs) 
{
		...
		for (size_t i = 0; i < values.size(); ++i) 
		{
				...
				const double fg_part = fg_log_mixing + fg_distro.log_likelihood(values[i]);
				assert(finite(fg_part));
    
				const double bg_part = bg_log_mixing + bg_distro.log_likelihood(values[i]);
				assert(finite(fg_part));
				  ...
				  Distro &fg_distro, Distro &bg_distro) 
{
		...
		mixing = smithlab::log_sum_log_vec(log_fg_probs, log_fg_probs.size());
		const double bg_mixing = smithlab::log_sum_log_vec(log_bg_probs, log_bg_probs.size());
...
void
TwoStateResolveMixture(const vector<double> &values,
					   ...
					   double &mixing) 
{
		...
		vector<double> fg_part, bg_part;
		partition_values(values, fg_part, bg_part);
  
		// Obtain initial estimates of distribution values
		fg_distro.estimate_params_ml(fg_part);
		bg_distro.estimate_params_ml(bg_part);
		...
		{
				const double score = 
						expectation_step(values, mixing, fg_distro, bg_distro, fg_probs, bg_probs);
				maximization_step(values, fg_probs, bg_probs, mixing, fg_distro, bg_distro);
				if (VERBOSE) 
				{
...
{
    // there should be someway to implement EM considering scaling factors
    // here we just make an not quite accurate guess
    vector<double> vals(values.begin(), values.end());
    for (size_t i = 0; i < values.size(); ++i)
    {
        vals[i] /= scales[i];
        if (vals[i] - static_cast<int>(vals[i]) > 0.5)
            vals[i] = static_cast<int>(vals[i]) + 1;
        else
            vals[i] = static_cast<int>(vals[i]);
    }
    
    // call the simpler one 
    TwoStateResolveMixture(vals, max_iterations, tolerance, VERBOSE,
                           fg_distro, bg_distro, mixing);
}
