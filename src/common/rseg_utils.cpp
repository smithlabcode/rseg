...
pick_training_sample(
  const vector<double> &read_bins, const vector<double> &read_bins_a,
  const vector<double> &read_bins_b, const vector<double> &scales,
  const vector<size_t> &reset_points, const size_t training_sample_size,
  vector<double> &read_bins_sample, vector<double> &read_bins_a_sample,
  vector<double> &read_bins_b_sample, vector<double> &scales_sample,
...
set_transitions(const size_t bin_size, const double fg_size,
                const vector<double> &mixing, vector<double> &start_trans,
...
void
set_transitions(const size_t bin_size, const double fg_size,
                const double mixing, vector<double> &start_trans,
                vector<vector<double>> &trans, vector<double> &end_trans) {
...
report_final_values(const vector<Distro> &distros,
  ...
  cout << "FINAL ESTIMATES" << endl << "---------------" << endl;
...
write_read_counts_by_bin(
  const vector<vector<SimpleGenomicRegion>> &bin_boundaries,
  const vector<double> &read_bins, const vector<double> &scales,
  const vector<bool> &classes, const string &file_name, const bool VERBOSE) {
  ...
  for (size_t i = 0; i < bin_boundaries.size(); ++i)
    for (size_t j = 0; j < bin_boundaries[i].size(); ++j) {
      outf << bin_boundaries[i][j] << "\t" << read_bins[k] << "\t" << scales[k]
           << "\t" << classes[k] << std::endl;
      ++k;
    }
  ...
  outf.close();
  if (VERBOSE)
    cout << "Read count file: " << file_name << std::endl;
}
...
write_read_counts_by_bin(
  const vector<vector<SimpleGenomicRegion>> &bin_boundaries,
  const vector<double> &read_bins, const vector<double> &read_bins_a,
  const vector<double> &read_bins_b, const vector<bool> &classes,
  const string &file_name, const bool VERBOSE) {
  std::ofstream outf(file_name.c_str());
  ...
  for (size_t i = 0; i < bin_boundaries.size(); ++i)
    for (size_t j = 0; j < bin_boundaries[i].size(); ++j) {
      outf << bin_boundaries[i][j] << "\t" << read_bins[k] << "\t"
...
write_read_counts_by_bin(
  const vector<vector<SimpleGenomicRegion>> &bin_boundaries,
  const vector<double> &read_bins, const vector<double> &read_bins_a,
  const vector<double> &read_bins_b, const vector<size_t> &classes,
  const string &file_name, const bool VERBOSE) {
  std::ofstream outf(file_name.c_str());
...
void
write_wigfile(const vector<double> &fg_scores, const vector<double> &bg_scores,
              const vector<vector<SimpleGenomicRegion>> &bin_bounds,
              const string &wigfile_name) {
  std::ofstream wigout(wigfile_name.c_str());
...
write_bed_file(const vector<vector<GenomicRegion>> &regions,
  ...
  for (vector<vector<GenomicRegion>>::const_iterator i = regions.begin();
       i != regions.end(); ++i)
...
build_domains(
  ...
  const double score_cutoff, vector<vector<GenomicRegion>> &domains,
  ...
  for (size_t i = 0; i < bins.size(); ++i) {
    ...
    while (start < lim) {
      ...
      const int fg_n =
        std::accumulate(classes[i].begin() + start, classes[i].begin() + end,
                        static_cast<int>(0));
      size_t label = (fg_n * 2 >= end - start) ? FG_LABEL : BG_LABEL;

      const size_t pre_label = (start > 0) ? labels[i][start - 1] : UN_LABEL;
      const size_t next_label = (end < lim) ? labels[i][end] : UN_LABEL;
...
build_domains(
  ...
  for (size_t i = 0; i < bins.size(); ++i) {
    ...
    while (start < lim) {
      ...
      size_t label =
        std::max_element(label_bin_nums.begin(), label_bin_nums.end()) -
...
struct IsEnrichedDomain : public std::unary_function<GenomicRegion, bool> {
  bool
  operator()(const GenomicRegion &dom) const {
...
pick_domains(const vector<vector<SimpleGenomicRegion>> &bins,
  ...
  for (size_t i = 0; i < domains.size(); ++i)
    domains[i].erase(std::stable_partition(domains[i].begin(), domains[i].end(),
                                           IsEnrichedDomain()),
                     domains[i].end());
}
...
pick_domains(const vector<vector<SimpleGenomicRegion>> &bins,
  ...
  for (size_t i = 1; i < fg_cdfs.size(); ++i) {
    fg_cdfs[i] =
      fg_cdfs[i - 1] + exp(distros.front().log_likelihood(i + offset));
    bg_cdfs[i] =
      bg_cdfs[i - 1] + exp(distros.back().log_likelihood(i + offset));
  }
