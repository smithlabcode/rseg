...
SplitDistro::SplitDistro(const std::string &n,
                         const std::vector<double> &params) :
  name(n), d(split_distro_factory(name)) {
  d->set_params(params);
}
...
SplitDistro::SplitDistro(const SplitDistro &rhs) :
  name(rhs.name), d(split_distro_factory(rhs.name)) {
...
SplitDistro::~SplitDistro() {
  if (d)
    delete d;
}

double
SplitDistro::operator()(double val) const {
  return (*d)(val);
}
...
split_distro_factory(string name, string params) {
  ...
  if (params_split.size() != distro->required_params())
    throw std::runtime_error(
      "bad number of params: " + smithlab::toa(params_split.size()) +
...
split_distro_factory(string name_arg) {
  ...
  if (name == "skel")
    ...
    throw std::runtime_error("bad split distribution name \"" + name + "\"");

  if (name_split.size() > 1) {
    vector<string> params_split(
      vector<string>(name_split.begin() + 1, name_split.end()));
    if (params_split.size() != distro->required_params())
      throw std::runtime_error(
        "bad number of params: " + smithlab::toa(params_split.size()) +
        " for split distro " + name + "\"");
...
double
SkellamDistro::log_likelihood(const double val, const double scale) const {
  if (std::fabs(val) > 120)
    return -40;
...
SkellamDistro::estimate_params_ml(const vector<double> &vals_a,
  ...
  params.front() =
    exp(split_distro_log_sum_log_vec(workspace_vals_a, lim)) / prob_sum;
  params.back() =
    exp(split_distro_log_sum_log_vec(workspace_vals_b, lim)) / prob_sum;
}
...
double
GaussianDistro::log_likelihood(const double val, const double scale) const {
...
GaussianDistro::estimate_params_ml(const vector<double> &vals_a,
  ...
  params.back() =
    gsl_stats_sd_m(&vals_c.front(), 1, vals_c.size(), params.front());
...
GaussianDistro::estimate_params_ml(const vector<double> &vals_a,
  ...
  for (size_t i = 0; i < vals_c.size(); ++i)
    vals_c[i] -= vals_b[i];
  params.front() = gsl_stats_wmean(&probs.front(), 1, &vals_c.front(), 1, lim);
  params.back() =
    gsl_stats_wsd_m(&probs.front(), 1, &vals_c.front(), 1, lim, params.front());
  //  cerr << params.front() << "\t" << params.back() << endl;
}
    ...
    while (fabs((prev - sum) / sum) > tolerance) {
      ...
      const double log_form =
        (gsl_sf_lngamma(k + n + r1) -
         gsl_sf_lnfact(static_cast<size_t>(k + n)) + gsl_sf_lngamma(n + r2) -
         gsl_sf_lnfact(static_cast<size_t>(n))) +
        n * c;
...
double
NegBinomDiffDistro::log_likelihood(const double k, const double scale) const {

  const double scaled_p1 = r1 / (r1 + params[0] * scale);
  const double scaled_p2 = r2 / (r2 + params[2] * scale);
  const double scaled_log_q1_log_q2 = log(1 - scaled_p1) + log(1 - scaled_p2);
  const double scaled_r1_log_p1_M_lnG_r1_P_r2_log_p2_M_lnG_r2 =
    (r1 * log(scaled_p1) - gsl_sf_lngamma(r1) + r2 * log(scaled_p2) -
     gsl_sf_lngamma(r2));
  ...
  while (fabs((prev - sum) / sum) > tolerance) {
    prev = sum;
    const double log_form =
      (gsl_sf_lngamma(k + n + r1) - gsl_sf_lnfact(static_cast<size_t>(k + n)) +
       gsl_sf_lngamma(n + r2) - gsl_sf_lnfact(static_cast<size_t>(n))) +
      n * scaled_log_q1_log_q2;
    sum = ((sum == 0) ? log_form : log_sum_log(sum, log_form));
    ++n;
  }

  sum +=
    k * log(1 - scaled_p1) + scaled_r1_log_p1_M_lnG_r1_P_r2_log_p2_M_lnG_r2;
  return sum;
}
...
estimate_params_pair_ml(const vector<double> &vals, double &mu, double &alpha) {
  ...
  //   const double var = gsl_stats_variance_m(&vals.front(), 1, vals.size(),
  //   mu); const double r = (mu*mu)/(var - mu); alpha = max(min_alpha, 1/r);
}

static void
estimate_params_pair_ml(const vector<double> &vals, const vector<double> &probs,
                        vector<double> &workspace_vals,
                        vector<double> &workspace_probs, double &mu,
                        double &alpha) {
  const size_t lim = vals.size();
  ...
  const double vals_count =
    exp(SplitDistro::log_sum_log_vec(workspace_probs, lim));
...
local_log_likelihood(const vector<double> &vals, const vector<double> &probs,
                     const double &mu1, const double &alpha1, const double &mu2,
                     const double &alpha2) {
...
double
local_log_likelihood(const vector<double> &vals, const double &mu1,
                     const double &alpha1, const double &mu2,
                     const double &alpha2) {
  const vector<double> probs(vals.size(), 1);
  return local_log_likelihood(vals, probs, mu1, alpha1, mu2, alpha2);
}
...
NegBinomDiffDistro::hq_estimate_params_ml(const std::vector<double> &vals_a,
  ...
  // get initial value
  estimate_params_pair_ml(vals_a, probs, workspace_vals_a, workspace_probs, mu1,
                          alpha1);
  estimate_params_pair_ml(vals_b, probs, workspace_vals_b, workspace_probs, mu2,
                          alpha2);
  ...
  while (fabs(llh_diff / curr_llh) > max_allow_llh_diff) {
    ...
    double local_scale = min(
      min(mu1 / fabs(mu1_grad + 1e-20), alpha1 / fabs(alpha1_grad + 1e-20)),
      min(mu2 / fabs(mu2_grad + 1e-20), alpha2 / fabs(alpha2_grad + 1e-20)));
    ...
    curr_llh =
      local_log_likelihood(vals, probs, mu1 + mu1_grad, alpha1 + alpha1_grad,
                           mu2 + mu2_grad, alpha2 + alpha2_grad);
    llh_diff = curr_llh - prev_llh;

    while (llh_diff <
           -1e-100)  // if the move can not lead to greater likelihood
...
double
local_log_likelihood(const vector<double> &vals, const vector<double> &scales,
                     const vector<double> &probs, const double &mu1,
                     const double &alpha1, const double &mu2,
                     const double &alpha2) {
  static const double tolerance = 1e-160;
...
NegBinomDiffDistro::estimate_params_ml(const std::vector<double> &vals_a,
                                       ...
                                       const std::vector<double> &scales,
                                       const std::vector<double> &probs) {
  estimate_params_pair_ml(vals_a, scales, probs, workspace_vals_a,
                          workspace_probs, params[0], params[1]);
  estimate_params_pair_ml(vals_b, scales, probs, workspace_vals_b,
                          workspace_probs, params[2], params[3]);
}
