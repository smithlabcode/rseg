...
public:
  ...
  Distro_ &
  operator=(const Distro_ &);
  virtual ~Distro_();

  virtual size_t
  required_params() const = 0;
  virtual void
  estimate_params_ml(const std::vector<double> &) = 0;
  virtual void
  estimate_params_ml(const std::vector<double> &vals,
...
public:
  ...
  log_sum_log_vec(const std::vector<double> &vals, size_t limit);
  static bool
  has_params(const std::string &name);
...
public:
  ...
  NegBinomDistro &
  operator=(const NegBinomDistro &rhs);
  ~NegBinomDistro() {}
  void
  set_helpers();
  size_t
