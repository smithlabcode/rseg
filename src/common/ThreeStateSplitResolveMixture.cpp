...
#include "ThreeStateSplitResolveMixture.hpp"
...
expectation_step(const vector<double> &values, 
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    const double mid_part = mid_log_mixing + mid_distro.log_likelihood(values[i]);
    if (!finite(mid_part)) {
...
void
ThreeStateSplitResolveMixture(const vector<double> &values,
			      const vector<double> &vals_a,
			      const vector<double> &vals_b,
  ...
  partition_values(values, vals_a, vals_b,
		   fg_part_a, fg_part_b, 
		   mid_part_a, mid_part_b, 
		   bg_part_a, bg_part_b);
  
  // Obtain initial estimates of distribution values
  fg_distro.estimate_params_ml(fg_part_a, fg_part_b);
  mid_distro.estimate_params_ml(mid_part_a, mid_part_b); 
  bg_distro.estimate_params_ml(bg_part_a, bg_part_b);
  ...
  for (size_t itr = 0; itr < max_iterations; ++itr) {
    const double score = 
      expectation_step(values, mixing, fg_distro, mid_distro, bg_distro, 
		       fg_probs, mid_probs, bg_probs);
    maximization_step(values, vals_a, vals_b, fg_probs, mid_probs, bg_probs, 
