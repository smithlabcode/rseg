...
static void
initialize_values(const vector<double> &values, const vector<double> &scales,
...
expectation_step(const vector<double> &values, const vector<double> &scales,
                 const double &mixing, const Distro &fg_distro,
                 const Distro &bg_distro, vector<double> &fg_probs,
...
static void
maximization_step(const vector<double> &values, const vector<double> &scales,
                  const vector<double> &fg_probs,
                  const vector<double> &bg_probs, double &mixing,
                  Distro &fg_distro, Distro &bg_distro) {
...
TwoStateResolveMixture(const std::vector<double> &values,
  ...
  if (VERBOSE)
    cout << endl
         << std::setw(10) << "DELTA" << std::setw(14) << "(PARAMS,MIX)" << endl;
  ...
  for (size_t itr = 0; itr < max_iterations; ++itr) {
    ...
    maximization_step(values, scales, fg_probs, bg_probs, mixing, fg_distro,
                      bg_distro);
    if (VERBOSE) {
      cout << std::setw(10) << std::setprecision(4)
           << (prev_score - score) / prev_score << "\t" << std::setw(14)
           << fg_distro.tostring() << " " << std::setw(10) << mixing << " "
           << std::setw(14) << bg_distro.tostring() << " " << std::setw(10)
           << 1 - mixing << " " << endl;
    }
