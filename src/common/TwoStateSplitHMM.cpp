...
inline double
TwoStateSplitHMM::log_sum_log(const double p, const double q) const {
  if (p == 0) return q;
...
double
TwoStateSplitHMM::forward_algorithm(const vector<double> &vals,
  ...
  f[start].first = fg_distro.log_likelihood(vals[start]) + lp_sf;
  f[start].second = bg_distro.log_likelihood(vals[start]) + lp_sb;
  
  for (size_t i = start + 1; i < end; ++i) {
    const size_t k = i - 1;
    f[i].first = (fg_distro.log_likelihood(vals[i]) +
		  log_sum_log(f[k].first + lp_ff, f[k].second + lp_bf));
    f[i].second = (bg_distro.log_likelihood(vals[i]) + 
...
double
TwoStateSplitHMM::backward_algorithm(const vector<double> &vals,
				     const size_t start, const size_t end,
				     const double lp_sf, const double lp_sb,
				     const double lp_ff, const double lp_fb, 
				     const double lp_ft,
  ...
  for (size_t k = end - 1; k > start; --k) {
    size_t i = k - 1;
    const double fg_a = fg_distro.log_likelihood(vals[k]) + b[k].first;
    const double bg_a = bg_distro.log_likelihood(vals[k]) + b[k].second;
    b[i].first = log_sum_log(fg_a + lp_ff, bg_a + lp_fb);
    b[i].second = log_sum_log(fg_a + lp_bf, bg_a + lp_bb);
  }
  return log_sum_log(b[start].first + fg_distro.log_likelihood(vals[start]) + lp_sf,
		     b[start].second + bg_distro.log_likelihood(vals[start]) + lp_sb);
}



double
TwoStateSplitHMM::log_sum_log_vec(const vector<double> &vals, size_t limit) const {
...
void
TwoStateSplitHMM::estimate_emissions(const vector<pair<double, double> > &f,
...
void
TwoStateSplitHMM::estimate_transitions(const vector<double> &vals,
				       const size_t start, const size_t end,
...
double
TwoStateSplitHMM::single_iteration(const vector<double> &values,
...
double
TwoStateSplitHMM::BaumWelchTraining(const vector<double> &values,
				    const vector<double> &vals_a,
				    const vector<double> &vals_b,
				    const vector<size_t> &reset_points,
...
void
TwoStateSplitHMM::PosteriorScores(const vector<double> &values,
				  const vector<size_t> &reset_points,
...
TwoStateSplitHMM::PosteriorScores(const vector<double> &values,
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    llr_scores[i] = (classes[i]) ?
        exp(fg_state - denom) : exp(bg_state - denom);
  }
}
...
TwoStateSplitHMM::PosteriorScores(const vector<double> &values,
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    llr_scores[i] = (class_id) ?
        exp(fg_state - denom) : exp(bg_state - denom);
  }
}

void
TwoStateSplitHMM::TransitionPosteriors(const vector<double> &values,
				       const vector<size_t> &reset_points,
				       const vector<double> &start_trans, 
				       const vector<vector<double> > &trans, 
				       const vector<double> &end_trans, 
				       const SplitDistro &fg_distro,
				       const SplitDistro &bg_distro,
                       vector< vector< vector<double> > > &trans_scores) const {
  ...
  return TransitionPosteriors(values, reset_points,
			      ...
			      trans[0][0], trans[0][1], end_trans[0],
			      trans[1][0], trans[1][1], end_trans[1],
			      fg_distro, bg_distro, trans_scores);
}

void
TwoStateSplitHMM::TransitionPosteriors(
    ...
    vector< vector< vector<double> > > &trans_scores) const
{
    ...
    for (size_t i = 0; i < trans_scores.size(); ++i)
        for (size_t j = 0; j < trans_scores[i].size(); ++j)
            trans_scores[i][j].resize(values.size());
    ...
    for (size_t i = 0; i < values.size(); ++i)
    {
        ...
        {
            ++j;
            trans_scores[0][0][i] = 0;
            trans_scores[0][1][i] = 0;
            trans_scores[1][0][i] = 0;
            trans_scores[1][1][i] = 0;
        }
        else
        {
            const double fg_to_fg_state = forward[i - 1].first + lp_ff + // transition
                // emission for value i + 1
                fg_distro.log_likelihood(values[i]) + backward[i].first;
            const double fg_to_bg_state = forward[i - 1].first + lp_fb + 
                bg_distro.log_likelihood(values[i]) + backward[i].second;
            ...
            trans_scores[0][0][i] = exp(fg_to_fg_state - denom);
            trans_scores[0][1][i] = exp(fg_to_bg_state - denom);
            trans_scores[1][0][i] = exp(bg_to_fg_state - denom);
            trans_scores[1][1][i] = exp(bg_to_bg_state - denom);
        }
    }
}
...
double
TwoStateSplitHMM::PosteriorDecoding(const vector<double> &values,
				    const vector<size_t> &reset_points,
				    const vector<double> &start_trans, 
				    const vector<vector<double> > &trans, 
				    const vector<double> &end_trans, 
...
double
TwoStateSplitHMM::ViterbiDecoding(const vector<double> &values,
				  const vector<size_t> &reset_points,
				  const vector<double> &start_trans, 
				  const vector<vector<double> > &trans, 
				  const vector<double> &end_trans, 
  ...
  return ViterbiDecoding(values, reset_points, start_trans[0], start_trans[1],
			 trans[0][0], trans[0][1], end_trans[0],
			 trans[1][0], trans[1][1], end_trans[1],
...
TwoStateSplitHMM::ViterbiDecoding(const vector<double> &values,
  ...
  for (size_t i = 0; i < reset_points.size() - 1; ++i) {
    ...
    v.front().first = fg_distro.log_likelihood(values[reset_points[i]]) + lp_sf;
    v.front().second = bg_distro.log_likelihood(values[reset_points[i]]) + lp_sb;
