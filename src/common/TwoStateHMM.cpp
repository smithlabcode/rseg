...
double
TwoStateHMM::forward_algorithm(const vector<double> &vals,
...
double
TwoStateHMM::backward_algorithm(const vector<double> &vals,
				const size_t start, const size_t end,
				const double lp_sf, const double lp_sb,
				const double lp_ff, const double lp_fb, 
				const double lp_ft,
...
void
TwoStateHMM::estimate_transitions(const vector<double> &vals,
				  const size_t start, const size_t end,
...
double
TwoStateHMM::single_iteration(const vector<double> &values,
...
//   const double p_bb_new_estimate = exp(log_sum_log_vec(bb_vals, values.size() - 1));
  ...
  fg_distro.estimate_params_ml(values, fg_probs);
  bg_distro.estimate_params_ml(values, bg_probs);
...
double
TwoStateHMM::BaumWelchTraining(const std::vector<double> &values,
  ...
  return BaumWelchTraining(values, reset_points,
...
double
TwoStateHMM::BaumWelchTraining(const vector<double> &values,
			       const vector<size_t> &reset_points,
			       double &p_sf, double &p_sb,
			       double &p_ff, double &p_fb, double &p_ft,
			       double &p_bf, double &p_bb, double &p_bt,
...
void
TwoStateHMM::PosteriorScores(const vector<double> &values,
			     const vector<size_t> &reset_points,
...
TwoStateHMM::PosteriorScores(const vector<double> &values,
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    ...
    const double bg_state = forward[i].second + backward[i].second;
    const double denom = log_sum_log(fg_state, bg_state);
    llr_scores[i] = fg_class ?
        exp(fg_state - denom) : exp(bg_state - denom);
  }
}

void
TwoStateHMM::TransitionPosteriors(const vector<double> &values,
				  const vector<size_t> &reset_points,
				  const vector<double> &start_trans,
				  const vector<vector<double> > &trans, 
				  const vector<double> &end_trans,
...
double
TwoStateHMM::PosteriorDecoding(const vector<double> &values,
			       const vector<size_t> &reset_points,
			       const vector<double> &start_trans,
			       const vector<vector<double> > &trans, 
			       const vector<double> &end_trans,
...
double
TwoStateHMM::ViterbiDecoding(const vector<double> &values,
			     const vector<size_t> &reset_points,
			     const vector<double> &start_trans,
			     const vector<vector<double> > &trans, 
			     const vector<double> &end_trans,
