            ...
            while (goods < bin_size) 
            {
                if (ditr < dlim) 
                {
                    ...
                    else 
                    {
                        ...
                        {
                            const size_t new_goods = min(bin_size - goods, 
                                                         curr_dead_start - end);
                            goods += new_goods;
                            end += new_goods;
                        }
...
{
    assert(reads.size() == regions.size() &&
           dead_zones.size() == reads.size());
    bins.resize(reads.size());
    boundaries.resize(reads.size());
    dead_scales.resize(reads.size());

    for (size_t i = 0; i < regions.size(); ++i)
        BinReadsCorrectDeadZones(
            reads[i], dead_zones[i], regions[i].get_chrom(), 
            regions[i].get_start(), regions[i].get_end(),
            bin_size, max_dead_proportion,
            boundaries[i], bins[i], dead_scales[i]);
}
    ...
    vector<vector<double> > &read_counts)
{
    ...
    for (size_t i = 0; i < reads.size(); ++i)
    {
        ...
        BinReadsCorrectDeadZones(
            reads[i], dead_zones[i], chrom, start, end,
