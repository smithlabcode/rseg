...
AdjustBinSize(const std::vector<double> &old_read_bins,
              ...
              const std::vector<std::size_t> &old_reset_points,
              const std::size_t old_bin_size, std::vector<double> &read_bins,
  ...
  const std::size_t n_steps = bin_size / old_bin_size;
  reset_points.push_back(0);
  for (std::size_t i = 0; i < old_reset_points.size() - 1; ++i) {
    const std::size_t start = old_reset_points[i];
    const std::size_t end = old_reset_points[i + 1];
    std::size_t j = start;
    while (j + n_steps <= end) {
      ...
      nondead_scales.push_back(
        std::accumulate(old_nondead_scales.begin() + j,
                        old_nondead_scales.begin() + j + n_steps, 0.0) /
        n_steps);
      j += n_steps;
    }
    reset_points.push_back(read_bins.size());
  }
  assert(*std::max_element(nondead_scales.begin(), nondead_scales.end()) <=
         1.0);
  assert(*std::min_element(nondead_scales.begin(), nondead_scales.end()) >=
         0.0);
}
...
GetCorrectedReadCounts(const std::vector<double> &read_bins,
  ...
  assert(std::size(read_bins) == std::size(nondead_scales));
  ...
  for (std::size_t i = 0; i < read_bins.size(); ++i)
    if (nondead_scales[i] > 1 - max_dead_proportion) {
      ...
      corrected_read_bins.push_back(dist(gen) < frac_part ? std::ceil(corrected)
                                                          : floor_count);
    }
}
    ...
    while (j + n_steps <= end) {
      ...
      read_bins_a.push_back(
        std::accumulate(old_read_bins_a.begin() + j,
                        old_read_bins_a.begin() + j + n_steps, 0.0));

      read_bins_b.push_back(
        std::accumulate(old_read_bins_b.begin() + j,
                        old_read_bins_b.begin() + j + n_steps, 0.0));
...
RemoveDeserts(std::vector<SimpleGenomicRegion> &old_bin_boundaries,
  ...
  assert(std::size(bin_boundaries) == reset_points.back());
  assert(std::size(read_bins_a) == reset_points.back());
  assert(std::size(read_bins_b) == reset_points.back());
  assert(std::size(nondead_scales) == reset_points.back());
